...
# USAGE: ./build.sh
#
# DESCRIPTION: Build a new WWUZEN Dual theme. Run this script to replace
#   instances of "wwuzen_dual" in all theme php files with the name of the new
#   dual theme. The script will prompt for the target and replacement text,
...
#   appropriately.
#
#   The script will NOT set the human-readable theme name in "readme.md" or the
#   .info file. This must be done manually. The displayed department name
#   visible to users is also not hard-coded; this must be set in the appearance
#   settings for the theme once it has been deployed and enabled.
#
...
__main() {
  ...
  theme name in readme.md or the .info file. This must be done manually. The
  displayed department name visible to users is also not hard-coded;
  this must be set in the appearance settings for the theme once it has
  been deployed and enabled. Press Enter to continue or CTRL+C to quit."

  # Get the target text from the user
  read -p "Enter the function name to replace, typically wwuzen_dual: " target

  # Get the replacmeent text from the user
  read -p "Provide a new function name, such as wwuzen_dual_new: " replacement
